...
#$SIPP -trace_err -sf uas_CANCEL.xml -i 127.0.0.1 -p 5090 -r 400 -rp 85s -m 500 -l 400 -bg -skip_rlimit
#UAS_PID=$!
UAS_PID=$(ps aux | grep '[u]as_CANCEL.xml' | awk '{print $2}')
if [ "$UAS_PID" == "" ]; then
  exit -1
...
#$SIPP 127.0.0.1:5060 -trace_err -sf uac_CANCEL.xml -i 127.0.0.1 -p 5050 -r 400 -rp 85s -m 500 -l 400 -bg -skip_rlimit
#UAC_PID=$!
UAC_PID=$(ps aux | grep '[u]ac_CANCEL.xml' | awk '{print $2}')
if [ "$UAC_PID" == "" ]; then
  exit
